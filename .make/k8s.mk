...
#	@jobs=$$(kubectl get job --output=jsonpath={.items..metadata.name} -n $(KUBE_NAMESPACE)); kubectl wait job --for=condition=complete --timeout=120s $$jobs -n $(KUBE_NAMESPACE)
	@kubectl -n $(KUBE_NAMESPACE) wait --for=condition=ready -l app=oet,component=rest --timeout=120s pods || exit 1
	@date
...
#
# defines a function to copy the ./tests directory into the K8s TEST_RUNNER
...
k8s_test = tar -c tests/ | \
		...
		tar -czvf /tmp/test-results.tgz -C /app build && \
		echo '~~~~BOUNDARY~~~~' && \
